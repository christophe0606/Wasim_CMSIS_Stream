...
EMSCRIPTEN_KEEPALIVE
bool canPush(struct FreeQueue *queue,size_t block_length);
...
EMSCRIPTEN_KEEPALIVE void PrintQueueAddresses(struct FreeQueue *queue) {
  ...
  printf("channel_data[0]    : %p   uint: %zu\n", 
      queue->channel_data[0], (size_t)queue->channel_data[0]);
  printf("channel_data[1]    : %p   uint: %zu\n", 
      queue->channel_data[1], (size_t)queue->channel_data[1]);
